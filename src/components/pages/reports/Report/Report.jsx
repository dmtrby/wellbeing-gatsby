...
const Report = () => {
  const [cookies] = useCookies(COOKIE_NAMES.EMAIL);
  const { setLoading } = useContext(LoadingOverlayContext);
  const emailCookie = cookies[COOKIE_NAMES.EMAIL];

  const [{ data: getData, loading: getLoading, error: getError }] = useAxios({
    url: `http:${process.env.API_URL}/report?surveyId=default&userId=${emailCookie}`,
    method: 'GET',
  });
        ...
        <div className="margin-top-1">
          <div>
            <div className={clsx(s.report_row, 'row')}>
              <div className="col-xs-8 col-md-10 col-no-gutter" />
