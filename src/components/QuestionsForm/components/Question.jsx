...
const Question = ({ question }) => {
  ...
  const radioLegends = useMemo(() => (!!(width > BREAKPOINTS.md && !pointsLegend)), [width, pointsLegend]);
  let legendArray = [];
  if (!radioLegends) {
    if (pointsLegend) {
  ...
  return (
    <div className="margin-bottom-5">
      ...
      <div>
        <RadioGroup questionId={questionId} points={points} radioLegends={radioLegends} />
      </div>
      {legendArray && (
        <div className="display-flex flex-justify-between margin-top-2">
          {legendArray.map((legend, index) => <QuestionLegend key={index}>{legend}</QuestionLegend>)}
        </div>
      )}
    </div>
  );
