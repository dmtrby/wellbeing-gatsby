...
const QuestionsForm = ({ surveyBlocks, onSubmit, surveyId }) => {
  const questionsData = useMemo(() => findKeysInSurveyBlocks(surveyBlocks), [surveyBlocks]);
  const initialValues = useMemo(() => ({ ...questionsData, email: '', company: '' }), [questionsData]);
              ...
              return (
                <div key={surveyBlockIndex}>
                  <div>
                    ...
                    {questions.map((question, questionIndex) => <Question question={question} key={questionIndex} />)}
                  </div>
