...
const RadioButton = ({ name, children, checked, value }) => (
    <label
      ...
      className={clsx(s.radiogroup_button__label, checked && s.radiogroup_button__label_active)}
    >
            ...
            <div className={clsx(s.radiogroup_button, 'col', 'col-no-gutter')} key={index}>
              {radioLegends ? (
                <div className={clsx('row', 'flex-column')}>
                  <RadioButton name={questionId} checked={value.toString() === values[questionId]} value={value}>
                    {value}
                  </RadioButton>
